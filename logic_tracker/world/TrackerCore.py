...
class TrackerCore():
    ...
    def updateTracker(self) -> CurrentTrackerState:
        if self.player_id is None or self.multiworld is None:
            print("Player YAML not installed or Generator failed", file=sys.stderr)
        ...
        if invalid_items:
            print(invalid_items)
            print("Your datapackage is incorrect, please correct the apworld for "+str(self.game), file=sys.stderr)
            print("The Following items are unknown [" + ",".join(invalid_items)+"]", file=sys.stderr)
    ...
    def initalize_tracker_core(self,connected_cls:type[AutoWorld.World],raw_slot_data):
        ...
        if getattr(connected_cls, "ut_can_gen_without_yaml", False):
            ...
            if self.slot_name in self.launch_multiworld.world_name_lookup:
                ...
                if known_slots:
                    print(f"Player's Yaml not in tracker's list. Known players: {known_slots}", file=sys.stderr)
                else:
                    print(f"Player's Yaml not in tracker's list. All known players are Yaml-less", file=sys.stderr)
                return
        if self.multiworld:
